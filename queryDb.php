<?php
   ...
   function getProducts($searchTerm = null) {
      ...
      } else {                                                                //Change manufacturer to CATEGORY                  Added % on either side of search terms to better searching
         $sql ='SELECT * FROM PRODUCTS WHERE PRODUCTNAME LIKE "%'.$searchTerm.'%" OR CATEGORY LIKE "%'.$searchTerm.'%" OR DESCRIPTION LIKE "%'.$searchTerm.'%" OR PRICE LIKE "%'.$searchTerm.'%"';
      }
      $ret = $db->query($sql);
      $array = [];
      if(!$ret){
